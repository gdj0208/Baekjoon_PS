...
void MainClass::bfs() {
    ...
    for(y = 0; y < ver; y++) {
        for(x = 0; x < hor; x++) {
            if(maze[y][x] == person) { tmp.push_front(make_tuple(y, x, maze[y][x])); }
            else if(maze[y][x] == fire) { tmp.push_back(make_tuple(y, x, maze[y][x])); }
        }
    }
            ...
            for(int i = 0; i < 4; i++) {
                ...
                if(type == person) {
                    if(maze[next_y][next_x] == space) {
                        ...
                        tmp.push_front(make_tuple(next_y, next_x, person));
                    }
                }
                else if(type == fire) {
                    if(maze[next_y][next_x] == space or maze[next_y][next_x] == person) {
                        maze[next_y][next_x] = fire;
                        tmp.push_back(make_tuple(next_y, next_x, fire));
                    }
                }
            }
        }
